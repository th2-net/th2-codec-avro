...
class TestAvroCodec {
    private var schema: Schema = getSchema("big_schema.avsc")
    private var codec: AvroCodec = AvroCodec(mapOf(1 to schema), AvroCodecSettings())
    @Test
    fun `test full decode`() {
        schema = getSchema("big_schema.avsc")
        codec = AvroCodec(mapOf(1 to schema), AvroCodecSettings())
        val rawBytes =
        ...
        assertEquals(37, actualCountFields)
    }

    @Test
    fun `test decode encode`() {
        schema = getSchema("big_schema_without_union.avsc")
        codec = AvroCodec(mapOf(1 to schema), AvroCodecSettings())
        val rawBytes =
            DatatypeConverter.parseHexBinary(
                ""
            )
        val body = ByteString.copyFrom(rawBytes)
        val decodeGroup = decode(body)
        val actualCountFields = decodeGroup.messagesList[0].message.fieldsMap.size
        assertEquals(12, actualCountFields)
    ...
    fun `test decode encode logical types`() {
        ...
        val rawBytes =
            DatatypeConverter.parseHexBinary(
                "000000000114303132333435363738390403E888A30292F3BE03D2C4B3A31BE0BEB4CDF65D809E8AE2CCE6DD0592B3B2B7D75DD2C4DFE188F3DB05"
            )
        val body = ByteString.copyFrom(rawBytes)
        val decodeGroup = decode(body)
        val actualCountFields = decodeGroup.messagesList[0].message.fieldsMap.size
    ...
    fun generateAvroRandomDataBySchema() {
        ...
        val idToSchema = "123 : alias1, 568: alias2"

        val map = idToSchema.split(",").associate {
