...
class StandardAvroCodec(schemaIdToSchema: Map<Int, Schema>, settings: AvroCodecSettings) : AbstractAvroCodec(settings) {
    ...
    override fun decodeRawMessage(rawMessage: RawMessage, sessionAlias: String): Message {
        ...
        val decoder = DecoderFactory.get().binaryDecoder(
            bytes,
           AVRO_HEADER_SIZE, bytes.size - AVRO_HEADER_SIZE, null
        )
    ...
    override fun encodeMessage(parsedMessage: Message, sessionAlias: String): ByteString? {
        ...
        val messageType =
            checkNotNull(parsedMessage.metadata.messageType) { "Message type is required. Message ${parsedMessage.toJson()} does not have it" }
